...
export default function AdminDashboard() {
  ...
  const activeRate = pct(s.active_users, s.total_users)
  const candidateRate = pct(s.total_candidates, s.total_users)
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between gap-2 rounded-2xl bg-rose-50/90 px-3 py-2.5 ring-1 ring-rose-100/80">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="truncate text-sm font-semibold text-rose-900">Total users</span>
                </div>
                <span className="shrink-0 text-sm font-black text-rose-950">{s.total_users}</span>
              </div>
              <div className="flex items-center justify-between gap-2 rounded-2xl bg-emerald-50/90 px-3 py-2.5 ring-1 ring-emerald-100/80">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="truncate text-sm font-semibold text-emerald-900">Active users</span>
                </div>
                <div className="flex shrink-0 items-center gap-2">
                  <span className="text-sm font-black text-emerald-950">{s.active_users}</span>
              ...
              <div className="flex items-center justify-between gap-2 rounded-2xl bg-slate-100/90 px-3 py-2.5 ring-1 ring-slate-200/80">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <span className="truncate text-sm font-semibold text-slate-800">Inactive users</span>
                </div>
                <span className="shrink-0 text-sm font-black text-slate-900">{s.inactive_users}</span>
              </div>
            </div>
          </section>
          ...
          <section className={glass}>
            ...
            <p className="mb-4 text-xs font-medium text-slate-500">Activation and candidate share</p>
            <div className="flex flex-col items-stretch gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex flex-col items-center rounded-2xl border border-white/60 bg-white/40 px-4 py-3 text-center sm:items-start sm:text-left">
                ...
                <p className="mt-1 text-[11px] font-medium text-emerald-700">{activeRate}% accounts active</p>
              </div>
              <div className="flex flex-1 flex-wrap items-end justify-center gap-6 sm:justify-end">
                <MiniDonut valuePct={activeRate} color="#059669" label="Active rate" sub="Active ÷ users" />
                <MiniDonut valuePct={candidateRate} color="#6366f1" label="Candidates" sub="÷ all users" />
              </div>
            </div>
